...
int convolution( filtre_t choix, unsigned char tab[],unsigned char tab2[], int nbl,int nbc) {
  int i,j;
  unsigned char *tmp;
...
int main(int argc, char *argv[]) {
  ...
  for(i=0 ; i < nbiter ; i++){
    ...
    unsigned char *tmp;
    /* Allocation memoire du tampon intermediaire : */
    tmp = (unsigned char*) malloc(w * h_loc * sizeof(unsigned char));
    if (tmp == NULL) {
      printf("Erreur dans l'allocation de tmp dans convolution \n");
      return 1;
    }
    ...
    if (rank > 0) {
      MPI_Isend(ima_loc + w, 2 * w, MPI_UNSIGNED_CHAR, rank - 1, TAG_DATA, MPI_COMM_WORLD, &request);
    }
    if (rank < p - 1) {
      MPI_Isend(ima_loc + (h_loc - 3) * w , 2 * w, MPI_UNSIGNED_CHAR, rank + 1, TAG_DATA, MPI_COMM_WORLD, &request);
    }
    convolution(filtre, ima_loc + w, tmp + w, h_loc - 1, w);
    if (rank > 0) {
      MPI_Irecv(ima_loc, 2 * w, MPI_UNSIGNED_CHAR, rank - 1, TAG_DATA, MPI_COMM_WORLD, &request1);
      MPI_Wait(&request1, &status);
    }
    if (rank < p - 1) {
      MPI_Irecv(ima_loc + (h_loc - 2) * w, 2 * w, MPI_UNSIGNED_CHAR, rank + 1, TAG_DATA, MPI_COMM_WORLD, &request2);
      MPI_Wait(&request2, &status);
    }

    convolution(filtre, ima_loc, tmp, 3, w);
    convolution(filtre, ima_loc + (h_loc - 2) * w, tmp + (h_loc - 2) * w, 3, w);
    ima_loc = tmp;
